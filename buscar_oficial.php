...
if (empty($placa)) {
	header("location: alta_oficial.php");
}else{
	...
	if ($idtipo != 1) {
		echo "Esta placa no pertenece a un vehículo oficial";
		echo '<button type="submit"><a href="alta_oficial.php" >Volver</a></button>';	

	} else {
		$sql = "SELECT e.placa, e.idtipo, t.tipo as tipo FROM entradas e, tipovehiculo t
		WHERE e.idtipo = t.idtipo AND t.idtipo = 1 AND e.placa LIKE '%$placa%' ";
